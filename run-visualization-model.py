...
"""
Visualization of Patchy-San + Ego-Convolution
--------------------------------------------
[fixed] Patchy-San + 5 Ego-Convolution
[+] 1 Attention+1 Dense: to score importance of neighborhoods (for general classification layers)
"""
...
def parse_arg():
    ...
    parser.add_option('-g', dest='gpu_id', default='0', help='use which GPU')
    parser.add_option('-f', dest='gpu_frac', default='0.99', help='GPU memory utilization')
    parser.add_option('-k', dest='K', default='4', help='hyperparameter k(#neighbor) of Ego-Convolution')
    parser.add_option('-p', dest='PK', default='10', help='hyperparameter k(size of neighborhood) of Patchy-San')
    parser.add_option('-l', dest='loop', default='5', help='#rerun in each fold')
    parser.add_option('-L', dest='lr', default='1e-4', help='learing rate for training')
    parser.add_option('-b', dest='bsize', default='32', help='batch size for training')
    parser.add_option('-c', dest='FOLD', default='10', help='10-Fold CV by default')
...
def g_conv_bn(X, theta, shape):
    ...
    return XW
def model_6LAttention_Small(nV, dim_f, dim_a, dim_e, bsize=32, k=10, nY=2, pk=10):
    ...
    x = tf.reshape(g_conv_bn(x, cv6_theta, [1,bsize,nV,n6]), [bsize*nV, n6])
    # attention
    a = tf.reshape(tf.nn.sigmoid(tf.matmul(x, a_theta[0]) + a_theta[1]), [bsize, nV])
    x = tf.multiply(tf.reshape(x, [bsize, nV*n6]), tf.concat([a for i in range(n6)], axis=1))
    # fc
    out = tf.matmul(x, fc_theta[0]) + fc_theta[1]    
    return adj, nbr, nnlbl, att, label, out, params
def model_6LAttention(nV, dim_f, dim_a, dim_e, bsize=32, k=10, nY=2, pk=10):
    adj = tf.placeholder(tf.float32, [1,bsize,nV,pk*pk*dim_e])
    nnlbl = tf.placeholder(tf.float32, [1,bsize,nV,pk*dim_f])
    att = tf.placeholder(tf.float32, [1,bsize,nV,pk*dim_a])
    nbr = tf.placeholder(tf.int32, [1,bsize,nV,k]) # |V| x k
    ...
    # attention
    a = tf.reshape(tf.nn.sigmoid(tf.matmul(x, a_theta[0]) + a_theta[1]), [bsize, nV])
    x = tf.multiply(x, tf.concat([a for i in range(n6)], axis=1))
    # fc
    x = tf.nn.relu(tf.matmul(x, fc1_theta[0]) + fc1_theta[1])
    x = tf.nn.dropout(x, remain_rate)
    out = tf.matmul(x, fc2_theta[0]) + fc2_theta[1]
    return adj, nbr, nnlbl, att, label, out, params
def model_3LAttention(nV, dim_f, dim_a, dim_e, bsize=32, k=10, nY=2, pk=10):
    adj = tf.placeholder(tf.float32, [1,bsize,nV,pk*pk*dim_e])
    nnlbl = tf.placeholder(tf.float32, [1,bsize,nV,pk*dim_f])
    att = tf.placeholder(tf.float32, [1,bsize,nV,pk*dim_a])
    nbr = tf.placeholder(tf.int32, [1,bsize,nV,k]) # |V| x k
    ...
    x = g_conv_bn(x, cv2_theta, [1,bsize,nV,n2])
    x = tf.reshape(aggregate_nbr(nbr, x, k), [1,bsize,nV,k*n2])
    x = tf.reshape(g_conv_bn(x, cv3_theta, [1,bsize,nV,n3]), [bsize, nV*n3])
    # attention
    a = tf.reshape(tf.nn.sigmoid(tf.matmul(x, a_theta[0]) + a_theta[1]), [bsize, nV])
    x = tf.multiply(x, tf.concat([a for i in range(n3)], axis=1))
    # fc
    x = tf.nn.relu(tf.matmul(x, fc1_theta[0]) + fc1_theta[1])
    x = tf.nn.dropout(x, remain_rate)
    out = tf.matmul(x, fc2_theta[0]) + fc2_theta[1]
    return adj, nbr, nnlbl, att, label, out, params
def cv(name, lr=1e-3, bsize=32, max_epch=1000, patience=10, k=3, pk=10, gpu_frac=0.99, loop=5, FOLD=10, model_type='3L'):
    # load data
    ...
    nL = 3 if model_type == '3L' else 6
    X_adj, X_nbr, X_nn, X_att, Y_gt, Y_logit, params = egonet(nV+1, dim_f, dim_a, dim_e, bsize=bsize, nY=nY, k=k, pk=pk) 
    ...
    for cv_i in range(FOLD):
        ...
        for re in range(loop):
            st = time.time()
            # init weight
            sess.run([assign_op], feed_dict={v:p for v,p in zip(var_placeholders, trained_params[:nL*2] + [xavier_init_val(p.shape) for p in params_val[nL*2:]])})
            # train:test=9:1
        ...
        cPickle.dump(dump, open('{}-k{}-V_cv_hist.pkl'.format(name, k), 'wb'))
    cPickle.dump(dump, open('{}-k{}-V_cv_hist.pkl'.format(name, k), 'wb'))
    sess.close()
    print('[CV] val_acc={:.3f}, test_acc={:.3f}, time={:.3f}s'.format(np.mean(cv_vacc), np.mean(test_accs), np.mean(times)))
    return dump
